...
The mouseover is keyed on *home* (origin), so the visualiztion shows flows
...
models and their implementation details in PySAL. In thie example, we explore
the influences of population size, income, and race (at origin and destination
locations) on the total flow moving between origin and destination. Though this
is a toy example, this approach combines traditional interaction variables i.e.
gravity driven by population size) with an inquiry about social inequality.
Since D.C. is such a famously segregated city, it may be that predominantly
Black neighbrhoods are greater exporters than importers, like the spatial
mismatch hypothesis suggests
